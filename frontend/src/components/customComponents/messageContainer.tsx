...
function MessageContainer({message, isGroup}:MessageContainerProps) {
  ...
  return (
    ...
    <p className={`w-full ${message.sender._id == user?._id ? "text-end pr-4" : "text-start pl-4"}  text-sm text-gray-600 w-full`} >{isGroup&& message.sender.userName}</p>
        <div className={`max-w-1/2 my-1 mx-4 ${message.sender._id == user?._id ? "self-end rounded-ee-xl rounded-s-xl" : "self-start rounded-e-xl rounded-es-xl"} flex flex-col leading-1.5 px-4 py-2 border-gray-200 bg-gray-200 border-1 dark:border-none dark:bg-gray-700`}>
